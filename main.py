...
class DynamicFilterView(discord.ui.View):
    def __init__(self, selected_map=None):
        ...
        # إضافة قائمة اختيار الماب دائماً
        self.add_item(MapDropdown(selected_map))
        
        # إذا تم اختيار ماب، أضف قائمة الأغراض تحتها مباشرة
        if selected_map:
            self.add_item(ItemDropdown(selected_map))
        
        # إضافة زر إضافة العرض في النهاية
        add_btn = discord.ui.Button(label="➕ إضافة عرضك", style=discord.ButtonStyle.green, custom_id="add_btn")
...
class MapDropdown(discord.ui.Select):
    def __init__(self, selected_map):
        ...
        super().__init__(placeholder="🗺️ 1. اختر الماب...", options=options)

    async def callback(self, interaction: discord.Interaction):
        selected_map = self.values[0]
        # تحديث الرسالة بنفس الإمبد ولكن مع قائمة الأغراض الجديدة
        embed = discord.Embed(
...
class ItemDropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # حل مشكلة Interaction Failed: نبلغ ديسكورد أننا استلمنا الطلب وجاري العمل
...
@bot.tree.command(name="setup_trade", description="إرسال واجهة المقايضة")
@app_commands.checks.has_permissions(administrator=True)
async def setup_trade(interaction: discord.Interaction):
    embed = discord.Embed(
        title="🛒 مركز المقايضة الذكي",
        description="اختر الماب أولاً لتظهر لك خيارات البحث.",
        color=0x3498db
    )
    await interaction.channel.send(embed=embed, view=DynamicFilterView())
    await interaction.response.send_message("✅ تم الإعداد بنجاح.", ephemeral=True)
